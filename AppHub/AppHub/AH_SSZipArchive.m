...
@interface AH_SSZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end

@implementation AH_SSZipArchive
{
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(id<AH_SSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<AH_SSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
		  toDestination:(NSString *)destination
			  overwrite:(BOOL)overwrite
			   password:(NSString *)password
				  error:(NSError **)error
			   delegate:(id<AH_SSZipArchiveDelegate>)delegate
		progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
	  completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
	// Begin opening
	zipFile zip = ah_unzOpen((const char*)[path UTF8String]);
	if (zip == NULL)
	{
		NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"failed to open zip file"};
		NSError *err = [NSError errorWithDomain:@"AH_SSZipArchiveErrorDomain" code:-1 userInfo:userInfo];
		if (error)
		{
			*error = err;
		}
	...
	ah_unzGetGlobalInfo(zip, &globalInfo);

	// Begin unzipping
	if (ah_unzGoToFirstFile(zip) != UNZ_OK)
	{
		NSDictionary *userInfo = @{NSLocalizedDescriptionKey: @"failed to open first file in zip file"};
		NSError *err = [NSError errorWithDomain:@"AH_SSZipArchiveErrorDomain" code:-2 userInfo:userInfo];
		if (error)
		{
			*error = err;
		}
	...
	do {
		@autoreleasepool {
			if ([password length] == 0) {
				ret = ah_unzOpenCurrentFile(zip);
			} else {
				ret = ah_unzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSASCIIStringEncoding]);
			}
			...
			ret = ah_unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
			if (ret != UNZ_OK) {
				success = NO;
				ah_unzCloseCurrentFile(zip);
				break;
			}
			...
			ah_unzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
	        ...
	        if (nil != err) {
	            NSLog(@"[AH_SSZipArchive] Error: %@", err.localizedDescription);
	        }
			...
			if (!fileIsSymbolicLink) {
	            ...
	            while (fp) {
	                int readBytes = ah_unzReadCurrentFile(zip, buffer, 4096);
	            ...
	            if (fp) {
	                ...
	                if (fileInfo.dosDate != 0) {
	                    ...
	                    if (attr) {
	                        if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
	                            ...
	                            NSLog(@"[AH_SSZipArchive] Failed to set attributes - whilst setting modification date");
	                        }
	                    }
	                }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[AH_SSZipArchive] Failed to set attributes - whilst setting permissions");
                        }
            ...
            else
            {
                ...
                int bytesRead = 0;
                while((bytesRead = ah_unzReadCurrentFile(zip, buffer, 4096)) > 0)
                {
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
            NSLog(@"[AH_SSZipArchive] Set attributes failed for directory: %@.", d[@"path"]);
        }
        if (err) {
            NSLog(@"[AH_SSZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
...
{
	BOOL success = NO;
	AH_SSZipArchive *zipArchive = [[AH_SSZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
...
- (BOOL)open
{
	...
	_zip = ah_zipOpen([_path UTF8String], APPEND_STATUS_CREATE);
	return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName
{
	...
	unsigned int len = 0;
    ah_zipOpenNewFileInZip(_zip, [[folderName stringByAppendingString:@"/"] UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_NO_COMPRESSION);
	ah_zipWriteInFileInZip(_zip, &len, 0);
	ah_zipCloseFileInZip(_zip);
	return YES;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName
{
    ...
    ah_zipOpenNewFileInZip(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
    ...
    while (!feof(input))
    {
		...
		ah_zipWriteInFileInZip(_zip, buffer, len);
	}
...
- (BOOL)writeData:(NSData *)data filename:(NSString *)filename
{
	...
	ah_zipOpenNewFileInZip(_zip, [filename UTF8String], &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_DEFAULT_COMPRESSION);

    ah_zipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
- (BOOL)close
{
	NSAssert((_zip != NULL), @"[AH_SSZipArchive] Attempting to close an archive which was never opened");
	ah_zipClose(_zip, NULL);
	return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[AH_SSZipArchive] MSDOS date masks don't add up");
